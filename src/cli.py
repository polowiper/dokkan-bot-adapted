...
def execute(user_input: str):
    for command_name in CommandService.get_names():
        ...
        if not is_current or config.game_context not in command.CONTEXT:
            continue
...
def run():
    CommandService.load()
    commands.help.run()  # Change this depending on your save name fr I just did that to automate the loading process
